...
class Model(classes.CommonModel.CommonModel):
    def __init__(self):
        pass

    def _modelInternal(self, _args):
        if classes.CommonModel.CommonModel.Kmodel is None:
            print('No model file available. Must load a model file first')
            return
        if _args.outFile is None:
            if _args.summary:       classes.CommonModel.CommonModel.Kmodel.summary()
            if _args.configuration: print(classes.CommonModel.CommonModel.Kmodel.get_config())
            if _args.weights:       print(classes.CommonModel.CommonModel.Kmodel.get_weights())
        else:
            try:
