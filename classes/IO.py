...
class IO(cm.CommonModel):
    ...
    def _run(self, _args):
        ...
        shouldReturn = False
        if self._inputData.shape != cm.CommonModel.Kmodel.layers[self._inputLayerNumber].input_shape:
                print('Error: Shape {} of data in input-file MUST be equal to shape {} of input in layer {}'.format(
        ...
        if (self._exptdOutput is not None) and (len(self._outputLayerNumbers) != 1):
            print('Error: The expected-output-file MUST be associated with only one output-layer {}'.format(
    ...
    def execute(self, _line):
        ...
        _ioPsSetup.add_argument('inFile', action='store', 
                help='path of the file that has the input to the model; the file suffix must be ".npy" and it  must be a numpy array')
        ...
        _ioPsSetup.add_argument('--exptdOutFile', '-ef', action='store', metavar='fileName',
                help='path of the file that has the expected output of the last layer only;  the file suffix must be ".npy" and it  must be a numpy array')
